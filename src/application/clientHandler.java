...
public class clientHandler implements Runnable {
	public clientHandler(Socket socket, ObservableList<Usuario> usuarioList, List<Sala> listaDeSala, Crypto crypt)
			throws IOException {
	...
	public void run() {
		...
		try {
			while ((cryptomensagem = (CryptoMensagem) in.readObject()) != null) {
				...
				if (tipo.equals(Tipo.ABRIRCONEXAO)) { // *********
				...
				} else if (tipo.equals(Tipo.DESCONECTARTOTAL)) { // tratado no
																	// controller
					...
					enviarParaTodos(mensagem,
							usuario.getSalaAtivaUsuario()
									.get(usuario.getSalaAtivaUsuario().indexOf(new Sala(mensagem.getSala(), null)))
									.getUsuarioList());
					...
					for (Sala sala : usuario.getSalaAtivaUsuario()) {
						if (sala.getId() == mensagem.getSala()) {
							if (sala.getAdministrador() != null
									&& sala.getAdministrador().getNome().get().equals(mensagem.getNome())) {
								Usuario usuarioTemp = sala.getUsuarioList()
										.get(sala.getUsuarioList().indexOf(new Usuario(mensagem.getNameReserved())));
	...
	private void desconectarSala(Mensagem mensagem) {
		...
		listaUsuarioMensagem = usuario.getSalaAtivaUsuario()
				.get(this.usuario.getSalaAtivaUsuario().indexOf(new Sala(mensagem.getSala(), null))).getUsuarioList();
	...
	private void enviarOnline(List<Usuario> usuarioList, int sala) {
